...
export default function HomePage() {
  ...
  // kept for type-safety even though not used as a click handler directly
  ...
  return (
    <>
            ...
            {loggedInUser ? (
              <div style={{ textAlign: 'center' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                  <Link href="/vault" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, background: 'var(--gold)', color: 'var(--black)', textDecoration: 'none', padding: '12px 20px', borderRadius: 8, fontSize: 14, fontWeight: 700 }}>
                    View My Vault →
                  </Link>
                  <Link href="/redeem" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, background: 'rgba(212,160,23,0.12)', border: '1px solid rgba(212,160,23,0.3)', color: 'var(--gold)', textDecoration: 'none', padding: '12px 20px', borderRadius: 8, fontSize: 14, fontWeight: 600 }}>
                    ⭐ Redeem Points
                  </Link>
                </div>
              </div>
                ...
                <div style={{ position: 'relative', marginBottom: 12 }}>
                  <input
                    ...
                    style={{
                      ...
                      border: `1px solid ${loginError ? 'rgba(230,57,70,0.5)' : 'rgba(255,255,255,0.12)'}`,
                      borderRadius: 8,
                      padding: '12px 14px',
                      color: 'white',
      ...
      {/* ── PRODUCTS SECTION with tab toggle ── */}
      <section className="section-pad" style={{ background: 'white' }}>
        ...
        <div className="catalog-header">
          ...
          <Link href={productTab === 'sealed' ? '/catalog' : '/redeem'} style={{ fontSize: 13, color: 'var(--blue2)', textDecoration: 'none', fontWeight: 500 }}>
            View All →
          </Link>
        </div>
        ...
        <div className="product-section-tabs">
          <button onClick={() => setProductTab('sealed')} style={{
            padding: '10px 22px', fontSize: 13, fontWeight: 600, cursor: 'pointer', border: 'none',
            background: productTab === 'sealed' ? 'var(--black)' : 'white',
            color: productTab === 'sealed' ? 'white' : '#8892A8',
            transition: 'all 0.2s',
          }}>🃏 Sealed</button>
          <button onClick={() => setProductTab('redeem')} style={{
            padding: '10px 22px', fontSize: 13, fontWeight: 600, cursor: 'pointer', border: 'none',
            background: productTab === 'redeem' ? 'var(--gold)' : 'white',
            color: productTab === 'redeem' ? 'var(--black)' : '#8892A8',
            transition: 'all 0.2s',
          }}>⭐ Redeemable</button>
        </div>
            ...
            <div className="grid-4">
              {redeemProducts.map(p => (
                <Link key={p.id} href={`/redeem/${p.id}`} style={{ textDecoration: 'none' }}>
                  <div style={{ background: 'white', border: '1px solid #E4E8F0', borderRadius: 12, overflow: 'hidden', cursor: 'pointer', transition: 'all 0.25s' }}
                    onMouseEnter={e => { const el = e.currentTarget as HTMLDivElement; el.style.borderColor='var(--gold)'; el.style.transform='translateY(-3px)'; el.style.boxShadow='0 12px 40px rgba(212,160,23,0.15)'; }}
                    onMouseLeave={e => { const el = e.currentTarget as HTMLDivElement; el.style.borderColor='#E4E8F0'; el.style.transform='none'; el.style.boxShadow='none'; }}
                  >
                    ...
                    <div style={{ padding: 16 }}>
                      <div style={{ fontSize: 14, fontWeight: 600, color: 'var(--black)', marginBottom: 4 }}>{p.name}</div>
                      <div style={{ fontSize: 11, color: '#8892A8', marginBottom: 12 }}>{p.category}</div>
                      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                        ...
                        {p.stock !== undefined && (
                          <div style={{
                            fontSize: 10,
                            fontWeight: 600,
                            color: p.stock === 0 ? 'var(--red)' : p.stock <= 5 ? 'var(--red)' : '#8892A8',
                          }}>
                            {p.stock === 0 ? '❌ Out of stock' : p.stock <= 5 ? `⚠️ ${p.stock} left` : `${p.stock} in stock`}
                          </div>
                        )}
                      </div>
                    </div>

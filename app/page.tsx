...
import { getProducts, getCustomer, getRedeemableProducts } from '@/lib/db';
...
type RedeemProduct = { id:string; name:string; category?:string; pointsCost:number; stock?:number; emoji?:string; badge?:string; imageUrl?:string; };
...
const FALLBACK_REDEEM: RedeemProduct[] = [
...
// ── Shared stock pill ──────────────────────────────────────────────
function StockPill({ stock, size = 'sm' }: { stock?: number; size?: 'sm' | 'xs' }) {
  if (stock === undefined) return null;
  const fs = size === 'xs' ? 9 : 10;
  if (stock === 0)  return <span style={{ fontSize:fs, fontWeight:700, color:'#E63946', background:'rgba(230,57,70,0.1)', border:'1px solid rgba(230,57,70,0.2)', padding:'2px 7px', borderRadius:20, whiteSpace:'nowrap' }}>✕ Out of stock</span>;
  if (stock <= 5)   return <span style={{ fontSize:fs, fontWeight:700, color:'#D97706', background:'rgba(217,119,6,0.1)',  border:'1px solid rgba(217,119,6,0.2)',  padding:'2px 7px', borderRadius:20, whiteSpace:'nowrap' }}>⚠ {stock} left</span>;
  return <span style={{ fontSize:fs, fontWeight:600, color:'#16a34a', background:'rgba(22,163,74,0.08)', border:'1px solid rgba(22,163,74,0.18)', padding:'2px 7px', borderRadius:20, whiteSpace:'nowrap' }}>{stock} in stock</span>;
  }

export default function HomePage() {
  const router = useRouter();
  const [products, setProducts] = useState<Product[]>([]);
  const [redeemProducts, setRedeemProducts] = useState<RedeemProduct[]>([]);
  const [activeImg, setActiveImg] = useState<Record<string,number>>({});
  const [productTab, setProductTab] = useState<'sealed'|'redeem'>('sealed');
  ...
  useEffect(() => {
    getProducts().then(data => setProducts((data as Product[]).slice(0, 4))).catch(() => {});
    getRedeemableProducts().then(data => {
      const items = data as RedeemProduct[];
      setRedeemProducts(items.length ? items.slice(0, 4) : FALLBACK_REDEEM);
    }).catch(() => setRedeemProducts(FALLBACK_REDEEM));
  }, []);
  ...
  async function handleLogin() {
    ...
    try {
      ...
      if (data) { localStorage.setItem(SESSION_KEY, username); setLoggedInUser(username); setLoginInput(''); }
      else setLoginError('Username not found. Contact PokeJoe to get access.');
    } catch { setLoginError('Something went wrong. Please try again.'); }
    setLoginLoading(false);
  }

  function handleLogout() {
    localStorage.removeItem(SESSION_KEY); setLoggedInUser(null); setLoginInput(''); setLoginError('');
  }

  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  function goToVault() { router.push('/vault'); }
  ...
  return (
    <>
      ...
      {/* ── HERO ── */}
      <section style={{ background:'var(--black)', minHeight:'100vh', display:'flex', flexDirection:'column', justifyContent:'center', position:'relative', overflow:'hidden', paddingTop:'var(--nav-h)', paddingBottom:0 }}>
        ...
        <div className="hero-main-layout" style={{ flex:1 }}>
          <div className="hero-content">
            <div style={{ fontSize:11, letterSpacing:'0.2em', textTransform:'uppercase', color:'var(--gold)', marginBottom:20, fontWeight:500 }}>Indonesia&apos;s Premier TCG Vault</div>
            <h1 className="hero-title">YOUR<br /><span style={{ color:'var(--gold)' }}>POKEMON</span><br /><span style={{ WebkitTextStroke:'2px rgba(255,255,255,0.25)', color:'transparent' }}>VAULT</span></h1>
            <p style={{ fontSize:16, color:'rgba(255,255,255,0.45)', lineHeight:1.7, maxWidth:480, marginBottom:40, fontWeight:300 }}>Rip &amp; ship, vault storage, and earn rewards with every purchase.</p>
          ...
          <div className="hero-login-card">
            {loggedInUser ? (
              <div style={{ textAlign:'center' }}>
                ...
                <div style={{ fontFamily:'var(--ff-display)', fontSize:28, color:'white', letterSpacing:'0.06em', marginBottom:2 }}>{loggedInUser.toUpperCase()}</div>
                <div style={{ fontSize:12, color:'rgba(255,255,255,0.25)', marginBottom:24 }}>Session active</div>
                <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
                  <Link href="/vault" style={{ display:'flex', alignItems:'center', justifyContent:'center', gap:8, background:'var(--gold)', color:'var(--black)', textDecoration:'none', padding:'12px 20px', borderRadius:8, fontSize:14, fontWeight:700 }}>View My Vault →</Link>
                  <Link href="/redeem" style={{ display:'flex', alignItems:'center', justifyContent:'center', gap:8, background:'rgba(212,160,23,0.12)', border:'1px solid rgba(212,160,23,0.3)', color:'var(--gold)', textDecoration:'none', padding:'12px 20px', borderRadius:8, fontSize:14, fontWeight:600 }}>⭐ Redeem Points</Link>
                </div>
              </div>
            ) : (
              <>
                ...
                <div style={{ position:'relative', marginBottom:12 }}>
                  <input type="text" value={loginInput} onChange={e => { setLoginInput(e.target.value); setLoginError(''); }} onKeyDown={e => e.key==='Enter' && handleLogin()} autoComplete="off" readOnly onFocus={e => e.target.removeAttribute('readonly')}
                    style={{ width:'100%', background:'rgba(255,255,255,0.06)', border:`1px solid ${loginError?'rgba(230,57,70,0.5)':'rgba(255,255,255,0.12)'}`, borderRadius:8, padding:'12px 14px', color:'white', fontFamily:'var(--ff-mono)', fontSize:16, outline:'none', boxSizing:'border-box', transition:'border-color 0.2s', WebkitTextFillColor:'white', caretColor:'var(--gold)' }}
                    onBlur={e => { if (!loginError) e.target.style.borderColor='rgba(255,255,255,0.12)'; }} />
                </div>
                {loginError && <div style={{ background:'rgba(230,57,70,0.1)', border:'1px solid rgba(230,57,70,0.2)', borderRadius:6, padding:'8px 12px', fontSize:12, color:'#FF6B75', marginBottom:12 }}>{loginError}</div>}
                <button onClick={handleLogin} disabled={loginLoading || !loginInput.trim()}
      ...
      <section className="section-pad" style={{ background:'white' }}>
        <div className="catalog-header">
          <div>
            <div style={{ fontSize:11, letterSpacing:'0.2em', textTransform:'uppercase', color:'var(--gold)', marginBottom:10, fontWeight:500 }}>
              {productTab==='sealed' ? 'Featured' : 'Rewards'}
            </div>
            <h2 style={{ fontFamily:'var(--ff-display)', fontSize:'clamp(36px,6vw,52px)', lineHeight:0.95 }}>
              {productTab==='sealed' ? 'SEALED PRODUCTS' : 'REDEEM WITH POINTS'}
            </h2>
          </div>
          <Link href={productTab==='sealed' ? '/catalog' : '/redeem'} style={{ fontSize:13, color:'var(--blue2)', textDecoration:'none', fontWeight:500 }}>View All →</Link>
        </div>

        <div className="product-section-tabs">
          <button onClick={() => setProductTab('sealed')} style={{ padding:'10px 22px', fontSize:13, fontWeight:600, cursor:'pointer', border:'none', background:productTab==='sealed'?'var(--black)':'white', color:productTab==='sealed'?'white':'#8892A8', transition:'all 0.2s' }}>🃏 Sealed</button>
          <button onClick={() => setProductTab('redeem')} style={{ padding:'10px 22px', fontSize:13, fontWeight:600, cursor:'pointer', border:'none', background:productTab==='redeem'?'var(--gold)':'white', color:productTab==='redeem'?'var(--black)':'#8892A8', transition:'all 0.2s' }}>⭐ Redeemable</button>
        </div>
        ...
        {productTab==='redeem' && (
          redeemProducts.length===0
            ? <div style={{ textAlign:'center', padding:40, color:'#8892A8', fontSize:14 }}>Loading rewards...</div>
            : <div className="grid-4">
                {redeemProducts.map(p => (
                  <Link key={p.id} href={`/redeem/${p.id}`} style={{ textDecoration:'none' }}>
                    <div style={{ background:'white', border:'1px solid #E4E8F0', borderRadius:12, overflow:'hidden', cursor:'pointer', transition:'all 0.25s', opacity: p.stock===0 ? 0.65 : 1 }}
                      onMouseEnter={e => { if(p.stock!==0){ const el=e.currentTarget as HTMLDivElement; el.style.borderColor='var(--gold)'; el.style.transform='translateY(-3px)'; el.style.boxShadow='0 12px 40px rgba(212,160,23,0.15)'; }}}
                      onMouseLeave={e => { const el=e.currentTarget as HTMLDivElement; el.style.borderColor='#E4E8F0'; el.style.transform='none'; el.style.boxShadow='none'; }}>
                      ...
                      <div style={{ padding:16 }}>
                        ...
                        <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', gap:8, flexWrap:'wrap' }}>
                          <div style={{ display:'flex', alignItems:'center', gap:6, background:'rgba(212,160,23,0.1)', border:'1px solid rgba(212,160,23,0.25)', padding:'5px 10px', borderRadius:8 }}>
                            <span style={{ fontSize:14 }}>⭐</span>
                            <span style={{ fontSize:15, fontWeight:700, color:'var(--gold)', fontFamily:'var(--ff-mono)' }}>{p.pointsCost}</span>
                            <span style={{ fontSize:10, color:'rgba(212,160,23,0.7)', fontWeight:500 }}>pts</span>
                          </div>
                          {/* ── STOCK ── */}
                          <StockPill stock={p.stock} />
                        </div>
      ...
      <style>{`
        ...
        .hero-main-layout { display:flex; align-items:center; padding-bottom:40px; gap:40px; }
        .hero-content { position:relative; padding:40px 60px 0 80px; flex:1; max-width:640px; }
        .hero-login-card { position:relative; flex-shrink:0; width:340px; margin-right:80px; background:rgba(255,255,255,0.04); border:1px solid rgba(255,255,255,0.1); border-radius:20px; padding:32px 28px; backdrop-filter:blur(20px); }
        @media (max-width: 900px) {
          .hero-main-layout { flex-direction:column; padding-bottom:32px; align-items:stretch; }
          .hero-content { padding:18px 20px 0; max-width:none; }
          .hero-login-card { width:auto; margin:10px 20px 0; }
        }
      `}</style>
    </>
  );

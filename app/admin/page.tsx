...
import { addPoints, setVaultActive, addVaultItem, upsertProduct } from '@/lib/db';
...
export default function AdminPage() {
  ...
  const loadProducts = useCallback(async () => {
    ...
    const snap = await gd(col(db, 'products'));
  ...
  return (
    <div style={{ background:'#080B10', minHeight:'100vh', display:'flex', flexDirection:'column' }}>
      ...
      <div style={{ display:'none' }} className="admin-topbar">
        ...
        {sidebarOpen && (
          <div style={{ background:'#060810', borderBottom:'1px solid rgba(255,255,255,0.05)', display:'flex', padding:'8px 12px', gap:4, overflowX:'auto' }}>
            {navItems.map(n => (
              <button key={n.key} onClick={()=>{ setTab(n.key); setSidebarOpen(false); }} style={{ background:tab===n.key?'rgba(212,160,23,0.12)':'transparent', border:tab===n.key?'1px solid rgba(212,160,23,0.3)':'1px solid rgba(255,255,255,0.08)', color:tab===n.key?'var(--gold)':'rgba(255,255,255,0.5)', padding:'8px 14px', borderRadius:8, fontSize:12, cursor:'pointer', whiteSpace:'nowrap' }}>
                {n.icon} {n.label}
              </button>
            ))}
            <button onClick={()=>setAuthed(false)} style={{ background:'transparent', border:'1px solid rgba(255,255,255,0.1)', color:'rgba(255,255,255,0.3)', padding:'8px 14px', borderRadius:8, fontSize:12, cursor:'pointer', marginLeft:'auto', whiteSpace:'nowrap' }}>Logout</button>
          </div>
        )}
      </div>
      ...
      <div style={{ display:'flex', flex:1 }}>
        {/* Desktop Sidebar */}
        <div className="admin-sidebar-desktop" style={{ background:'#060810', width:220, borderRight:'1px solid rgba(255,255,255,0.05)', display:'flex', flexDirection:'column', flexShrink:0 }}>
        ...
        <div style={{ flex:1, padding:'32px', overflowY:'auto', overflowX:'hidden' }}>
          ...
          {tab==='customers' && <>
            ...
            <div className="stats-grid">
              {[
                ...
                <div key={i} style={{ background:'rgba(255,255,255,0.03)', border:'1px solid rgba(255,255,255,0.06)', borderRadius:10, padding:20 }}>
                  ...
                  <div style={{ fontFamily:'var(--ff-display)', fontSize:'clamp(24px,4vw,36px)', color:s.color, lineHeight:1 }}>{s.val}</div>
                </div>
              ))}
            </div>
            ...
            <div style={{ background:'rgba(255,255,255,0.03)', border:'1px solid rgba(255,255,255,0.08)', borderRadius:10, padding:24, marginBottom:24 }}>
              ...
              {firstName && lastName && (
                <div style={{ fontSize:12, color:'rgba(255,255,255,0.3)', marginBottom:12 }}>
                  Preview: <span style={{ fontFamily:'var(--ff-mono)', color:'var(--gold)' }}>@{generateUsername(firstName, lastName)}</span>
                </div>
              )}
            ...
            <div style={{ background:'rgba(255,255,255,0.03)', border:'1px solid rgba(255,255,255,0.08)', borderRadius:10, padding:24, marginBottom:24 }}>
              ...
              <div className="pts-grid">
                ...
                <div className="pts-arrow" style={{ textAlign:'center', paddingBottom:2 }}><div style={{ fontSize:20, color:'var(--gold)' }}>→</div><div style={{ fontSize:10, color:'rgba(255,255,255,0.2)' }}>Rp 100K = 1pt</div></div>
            ...
            <div style={{ background:'rgba(255,255,255,0.03)', border:'1px solid rgba(255,255,255,0.06)', borderRadius:10, overflow:'hidden' }}>
              <div style={{ padding:'16px 20px', borderBottom:'1px solid rgba(255,255,255,0.06)' }}>
                <input placeholder="🔍  Search username, name or phone..." value={search} onChange={e=>setSearch(e.target.value)}
                  style={{ width:'100%', background:'rgba(255,255,255,0.06)', border:'1px solid rgba(255,255,255,0.1)', borderRadius:8, padding:'10px 14px', color:'white', fontFamily:'var(--ff-body)', fontSize:13, outline:'none', boxSizing:'border-box' }} />
              </div>
                ...
                return filtered.map((c,i) => (
                  <div key={c.id}>
                    ...
                    <div onClick={()=>setExpanded(expanded===c.id ? null : c.id)}
                      ...
                      style={{ cursor:'pointer', background:expanded===c.id?'rgba(212,160,23,0.04)':'transparent', transition:'background 0.15s' }}
                    ...
                    >
                      <div className="col-num" style={{ fontSize:12, color:'rgba(255,255,255,0.2)', fontFamily:'var(--ff-mono)' }}>{String(i+1).padStart(2,'0')}</div>
                      <div style={{ fontFamily:'var(--ff-mono)', fontSize:13, color:'var(--gold)', fontWeight:700 }}>@{c.username}</div>
                      <div className="col-name" style={{ fontSize:13, color:'white' }}>{c.firstName} {c.lastName}</div>
                      <div style={{ fontFamily:'var(--ff-mono)', fontSize:13, color:'var(--gold)', fontWeight:700 }}>{c.points} pts</div>
                      <div className="col-orders" style={{ fontSize:12, color:'rgba(255,255,255,0.4)' }}>{c.purchaseHistory?.length||0}</div>
                      <div onClick={e=>e.stopPropagation()}>
                        <button onClick={()=>setVaultActive(c.username,!c.vaultActive).then(()=>{ showToast(`Vault ${!c.vaultActive?'activated':'deactivated'}`); loadCustomers(); })}
                          style={{ background:c.vaultActive?'rgba(34,197,94,0.15)':'rgba(255,255,255,0.06)', border:c.vaultActive?'1px solid rgba(34,197,94,0.3)':'1px solid rgba(255,255,255,0.1)', color:c.vaultActive?'var(--green)':'rgba(255,255,255,0.4)', padding:'5px 12px', borderRadius:20, fontSize:11, cursor:'pointer', fontWeight:600 }}>
                      ...
                      <div style={{ padding:'16px 20px 20px', background:'rgba(212,160,23,0.03)', borderBottom:'1px solid rgba(255,255,255,0.06)' }}>
                        {editing===c.id ? (
                          <div style={{ maxWidth:600 }}>
                            <div className="new-customer-grid" style={{ marginBottom:12 }}>
                              {[
                                ...
                                <div key={fi}>
                                  ...
                                  <input value={f.val} onChange={e=>f.set(e.target.value)} style={{ width:'100%', background:'rgba(255,255,255,0.06)', border:'1px solid rgba(212,160,23,0.4)', borderRadius:8, padding:'10px 12px', color:'white', fontFamily:'var(--ff-body)', fontSize:13, outline:'none', boxSizing:'border-box' as const }} />
                                </div>
                              ))}
                            </div>
                          ...
                          <div style={{ maxWidth:600 }}>
                            <div className="new-customer-grid" style={{ marginBottom:12 }}>
                              {[
                                ...
                                <div key={fi} style={{ background:'rgba(255,255,255,0.04)', border:'1px solid rgba(255,255,255,0.07)', borderRadius:8, padding:'12px 16px' }}>
                                  <div style={{ fontSize:10, color:'rgba(255,255,255,0.3)', letterSpacing:'0.1em', textTransform:'uppercase', marginBottom:6 }}>{f.label}</div>
                              ...
                              <div style={{ display:'flex', gap:8, flexWrap:'wrap' }}>
                                ...
                                {deleting===c.id ? (
                                  <div style={{ display:'flex', alignItems:'center', gap:6 }}>
                                    <span style={{ fontSize:12, color:'#FF6B75' }}>Delete?</span>
                                    <button onClick={async()=>{
                                      const { deleteDoc, doc: fd } = await import('firebase/firestore');
                                      await deleteDoc(fd(db, 'customers', c.id));
                                      showToast(`@${c.username} deleted`);
                                      setDeleting(null); setExpanded(null); loadCustomers();
                                    }} style={{ background:'var(--red)', color:'white', border:'none', padding:'6px 12px', borderRadius:6, cursor:'pointer', fontSize:12, fontWeight:600 }}>Yes</button>
          ...
          {tab==='vault' && <>
            <div style={{ marginBottom:28 }}>
              <div style={{ fontFamily:'var(--ff-display)', fontSize:'clamp(24px,5vw,32px)', color:'white' }}>ADD VAULT ITEMS</div>
            ...
            <div style={{ background:'rgba(255,255,255,0.03)', border:'1px solid rgba(255,255,255,0.08)', borderRadius:10, padding:28, maxWidth:700 }}>
              ...
              >
                ...
                <input id="vault-photo-input" type="file" accept="image/*" multiple style={{ display:'none' }}
                  onChange={e=>{ const files=Array.from(e.target.files||[]); setVaultPhotos(prev=>[...prev, ...files.map(f=>({ file:f, preview:URL.createObjectURL(f), uploading:false }))]); e.target.value=''; }} />
              </div>
                ...
                <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:10, marginBottom:16 }}>
                  {vaultPhotos.map((p,i)=>(
                    <div key={i} style={{ position:'relative', aspectRatio:'1', borderRadius:8, overflow:'hidden', border:'1px solid rgba(255,255,255,0.1)' }}>
                      ...
                      {!p.uploading && <button onClick={()=>setVaultPhotos(prev=>prev.filter((_,j)=>j!==i))} style={{ position:'absolute', top:4, right:4, background:'rgba(230,57,70,0.8)', border:'none', color:'white', width:22, height:22, borderRadius:'50%', cursor:'pointer', fontSize:12 }}>×</button>}
                    </div>
                  ))}
                </div>
              )}
              ...
              <button onClick={async()=>{
                ...
                if(vaultPhotos.length > 0) {
                  ...
                  photoUrls = await Promise.all(vaultPhotos.map(async(p)=>{
          ...
          {tab==='products' && <>
            <div style={{ marginBottom:28 }}>
              <div style={{ fontFamily:'var(--ff-display)', fontSize:'clamp(24px,5vw,32px)', color:'white' }}>PRODUCTS</div>
            ...
            <div style={{ background:'rgba(255,255,255,0.03)', border:'1px solid rgba(255,255,255,0.08)', borderRadius:10, padding:28, maxWidth:700, marginBottom:32 }}>
              ...
              <div
                ...
                style={{ border:'2px dashed rgba(255,255,255,0.1)', borderRadius:10, padding:24, textAlign:'center', marginBottom:10, cursor:'pointer' }}
              ...
              {prodPhotos.length > 0 && (
                <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:10, marginBottom:16 }}>
                  {prodPhotos.map((p,i)=>(
                    <div key={i} style={{ position:'relative', aspectRatio:'1', borderRadius:8, overflow:'hidden', border:'1px solid rgba(255,255,255,0.1)' }}>
                      <img src={p.preview} alt="" style={{ width:'100%', height:'100%', objectFit:'cover' }} />
                      {!p.uploading && <button onClick={()=>setProdPhotos(prev=>prev.filter((_,j)=>j!==i))} style={{ position:'absolute', top:4, right:4, background:'rgba(230,57,70,0.8)', border:'none', color:'white', width:22, height:22, borderRadius:'50%', cursor:'pointer', fontSize:12 }}>×</button>}
                    </div>
                  ))}
                </div>
              )}
                        ...
                        {deletingProd===p.id ? (
                          <div style={{ display:'flex', alignItems:'center', gap:6 }}>
                            ...
                            <button onClick={async()=>{
                              ...
                              setDeletingProd(null); loadProducts();
      ...
      {toast && <div style={{ position:'fixed', bottom:24, right:24, background:'var(--black)', color:'white', padding:'12px 20px', borderRadius:8, fontSize:14, zIndex:9999, borderLeft:'3px solid var(--gold)', maxWidth:'calc(100vw - 48px)' }}>{toast}</div>}
      ...
      <style>{`
        @media (max-width: 768px) {
          ...
          .customer-row { grid-template-columns: 1fr auto !important; }
          .customer-row .col-num, .customer-row .col-orders { display: none !important; }
        }

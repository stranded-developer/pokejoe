...
// ── Stock badge — larger for detail pages ─────────────────────────
function StockBadge({ stock }: { stock?: number }) {
  if (stock === undefined) return null;

  let color: string, bg: string, border: string, label: string;
  if (stock === 0) {
    color='#E63946'; bg='rgba(230,57,70,0.1)'; border='rgba(230,57,70,0.25)'; label='✕ Out of Stock';
  } else if (stock <= 5) {
    color='#D97706'; bg='rgba(217,119,6,0.1)';  border='rgba(217,119,6,0.25)';  label=`⚠ Only ${stock} left`;
  } else if (stock <= 10) {
    color='#16a34a'; bg='rgba(22,163,74,0.08)'; border='rgba(22,163,74,0.2)';  label=`${stock} in stock`;
  } else {
    color='#16a34a'; bg='rgba(22,163,74,0.08)'; border='rgba(22,163,74,0.2)';  label='In Stock';
  }

  return (
    <div style={{ display:'inline-flex', alignItems:'center', gap:6, color, background:bg, border:`1px solid ${border}`, padding:'6px 14px', borderRadius:8, fontSize:13, fontWeight:700 }}>
      {label}
    </div>
  );
}
        ...
        <div className="product-grid">
          ...
          <div style={{ paddingTop:8 }}>
            ...
            <div style={{ display:'flex', alignItems:'baseline', gap:12, marginBottom:16, flexWrap:'wrap' }}>
              <div style={{ fontSize:32, fontWeight:700, color: outOfStock ? '#8892A8' : 'var(--blue)', fontFamily:'var(--ff-mono)' }}>Rp {product.price.toLocaleString('id-ID')}</div>
              {!outOfStock && <div style={{ background:'rgba(212,160,23,0.08)', border:'1px solid rgba(212,160,23,0.2)', color:'var(--gold)', fontSize:12, fontWeight:600, padding:'5px 12px', borderRadius:20 }}>Earn {Math.floor(product.price/100000)} pts</div>}
            </div>
            ...
            <div className="product-stats-grid">
              {[
                ...
                <div key={i} style={{ background:'#F2F4F8', borderRadius:8, padding:'14px 16px', textAlign:'center', display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', minHeight:72 }}>
                  <div style={{ fontFamily:'var(--ff-mono)', fontSize:18, fontWeight:700, color:(s as {color?:string}).color || 'var(--black)' }}>{s.val}</div>
            ...
            {outOfStock ? (
              <div style={{ background:'rgba(230,57,70,0.06)', border:'1px solid rgba(230,57,70,0.15)', borderRadius:8, padding:'14px 16px', fontSize:13, color:'#E63946', textAlign:'center', fontWeight:500 }}>
                This product is currently out of stock. Contact us on WhatsApp to be notified when it&apos;s back.
              </div>
            ) : (
